...
type multicastAdvertisement struct {
	...
	Port         uint16
	WebPort      uint16 // WSS/WTS port (0 if not available)
	Hash         []byte
}

func (m *multicastAdvertisement) MarshalBinary() ([]byte, error) {
	b := make([]byte, 0, ed25519.PublicKeySize+10+len(m.Hash))
	...
	b = binary.BigEndian.AppendUint16(b, m.WebPort)
...
func (m *multicastAdvertisement) UnmarshalBinary(b []byte) error {
	...
	if len(b) < ed25519.PublicKeySize+10 {
		return fmt.Errorf("invalid multicast beacon")
	}
	m.WebPort = binary.BigEndian.Uint16(b[6+ed25519.PublicKeySize : 8+ed25519.PublicKeySize])
	dl := binary.BigEndian.Uint16(b[8+ed25519.PublicKeySize : 10+ed25519.PublicKeySize])
	m.Hash = append(m.Hash[:0], b[10+ed25519.PublicKeySize:10+ed25519.PublicKeySize+dl]...)
	return nil
}
